...
namespace AT.ExcelData
{
    ...
    public class ExcelQuery
    {
        ...
        string GetHeaderColumnName(int rownum, int cellnum)
        {
            ICell headerCell = sheet.GetRow(rownum).GetCell(cellnum);
            if (headerCell != null)
                return headerCell.StringCellValue;
            return string.Empty;
        }
        ...
        /// <summary> Retrieves all first columns(aka. header column) which are needed to determine each type of a cell. </summary>
        ...
        /// <summary> Get String Form Columns. Check String Form Column with Tag "$" </summary>
        public bool[] GetStringFormColumns(int targetRow, int columnSize = 1, bool isIncludeEmptyCell = false)
        {
            bool[] resultColumns = new bool[columnSize];
            ...
            {
                ICell cell = row.GetCell(readIndex);
                if (cell == null) break;

                string item = string.Empty;
                try
                {
                    item = (string)ConvertFrom(cell, typeof(string));
                }
                catch (Exception e)
                {
                    string pos = string.Format("Row[{0}], Column[{1}]", targetRow, readIndex + 1);
                    Debug.LogError(string.Format("Excel File {0}. Sheet[{1}]Catched Deserialize Exception: {2} at {3}", this.filepath, sheet.SheetName, e.Message, pos));
                }
                ...
                if (item.First() == '$')
                    resultColumns[i] = true;
                else
                    resultColumns[i] = false;

                if (string.IsNullOrEmpty(item) && isIncludeEmptyCell == false)
                {
                    string pos = string.Format("Row[{0}], Column[{1}]", targetRow, readIndex + 1);
                    Debug.LogError(string.Format("Excel File {0}. Sheet[{1}] Empty Space Catched {2}", this.filepath, sheet.SheetName, pos));
                }
        ...
        public List<string> GetColumns(int startRow = 1, int columnIndex = 0, bool isIncludeEmptyCell = false)
        {
            List<string> result = new List<string>();
            ...
            foreach (IRow row in sheet)
            {
                ...
                if (cell == null && !isIncludeEmptyCell)  // skip empty cell
                {
                    result.Add(item);
                    continue;
                }
                ...
                {
                    string pos = string.Format("Row[{0}], Cell[{1}]", row.ToString(), columnIndex);
                    Debug.LogError(string.Format("Excel File {0}. Sheet[{1}]Catched Deserialize Exception: {2} at {3}", this.filepath, sheet.SheetName, e.Message, pos));
                }

                if (string.IsNullOrEmpty(item) && !isIncludeEmptyCell)
                    continue;
                
                if (item.Length > 0 && item.First() == '#')
                    continue;
        ...
        public string[] GetRow(int targetRow, int columnSize = 1, bool isIncludeEmptyCell = false)
        {
            IRow row = sheet.GetRow(targetRow);
            if (row == null) return null;
